...
{   srand(time(NULL));
    ...
    {
        cout << "Alien discover an health pack beneath the rock" << endl;
    }
    else
    {
...
void Move::alien_move(board &Board)
{
    ...
    do
    {
        int new_x = x_ + x;// Update position of the alien
        int new_y = y_ + y;
        if (Board.isInsideMap(new_x, new_y))
        {
            ...
            else if (Board.get_object(x_, y_) == 'r')
            {
